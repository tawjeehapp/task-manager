-- Require every task to have an assignee; default historical nulls to creator.
...
-- Excusal approval must reassign to a concrete user (never clear assignee).
...
BEGIN
  ...
  IF v_request.type = 'extension' THEN
    ...
    IF v_project_end IS NOT NULL AND v_request.requested_date > v_project_end THEN
  ...
  ELSIF v_request.type = 'excusal' THEN
    ...
    IF p_new_assignee = v_request.user_id THEN
      RAISE EXCEPTION 'CANNOT_REASSIGN_TO_EXCUSED'
        USING ERRCODE = 'P0001';
    END IF;
